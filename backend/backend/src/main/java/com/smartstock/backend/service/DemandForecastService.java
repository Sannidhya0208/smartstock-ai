...
public class DemandForecastService {
    ...
    public DemandForecastResponse forecastDemand(
        ...
        int totalStockOut = stockOutTransactions.stream()
                .mapToInt(StockTransaction::getQuantity)
                .sum();

        double averageDailyDemand =
                (double) totalStockOut / HISTORY_DAYS;

        int predictedDemand = (int) Math.ceil(
                averageDailyDemand * selectedForecastDays
        );

        int currentStock = inventory.getStockLevel();

        int recommendedReorderQuantity = Math.max(
                predictedDemand - currentStock,
                0
        );

        String stockRisk = determineStockRisk(
                currentStock,
                predictedDemand
        );
        ...
        return new DemandForecastResponse(
                ...
                currentStock,
                roundToTwoDecimals(averageDailyDemand),
